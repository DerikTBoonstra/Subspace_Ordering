...
# Simulation --------------------------------------------------------------
{
  ...
  config_L3_out <- sim_wrapper(d = d, mu, Sigma, n = 5000, ni = c(p + 1, 2*p, 5*p), nsims = 1000, dim.order = "t2", model = "lda")
  plan(sequential)
  # save(config_L3_out, file = "saved_results/config_L3_out.RData")
}
